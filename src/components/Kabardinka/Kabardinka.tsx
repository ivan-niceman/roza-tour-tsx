...
export default function Kabardinka({
  onOpenAdvice,
}: {
  onOpenAdvice: () => void;
}) {
  const kabardinkaHotels = ALL_HOTELS.filter((h) => h.city === 'kabardinka');
  const city = getCityData('kabardinka');
  const showNavigation = kabardinkaHotels.length > 1;

  return (
    <div className="hotel-container container">
      <h1 className="bus-tours-title">г. Кабардинка</h1>
      {city && (
        <CityNav
      ...
      <ul className="bus-tours-hotel-list grid-container">
        {kabardinkaHotels.map((hotel, index) => (
          <HotelItem
            key={hotel.id}
            hotel={hotel}
