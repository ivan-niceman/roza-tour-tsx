...
export default function Gelendzhik({
  ...
  const gelendzhikHotels = ALL_HOTELS.filter((h) => h.city === 'gelendzhik');
  const city = getCityData('gelendzhik');
  const showNavigation = gelendzhikHotels.length > 1;

  return (
    <div className="hotel-container container">
      <h1 className="bus-tours-title">г. Геленджик</h1>
      {city && (
        <CityNav
      ...
      <ul className="bus-tours-hotel-list grid-container">
        {gelendzhikHotels.map((hotel, index) => (
          <HotelItem
            key={hotel.id}
            hotel={hotel}
